...
class Blogpage extends Component{
    ...
    componentDidMount(){
         ...
         if(main_language>0){
          ...
          window.scrollTo(0,0);
              axios.get("http://185.154.53.105:1338/api/categories")
              .then(response=>{
                  this.setState({
      ...
      ffffforrrtuna(e){
       ...
       $(e.target).addClass('afterclick_button_class1') &&  $(e.target).removeClass('normal_button_class1')
        for(let i=0; i<this.state.elementskateg[e.target.id].subcategory.length; i++){
            ...
            axios.post("http://185.154.53.105:1338/api/posts",{sub_cat_name:this.state.elementskateg[e.target.id].subcategory[i].name, cat_name:this.state.elementskateg[e.target.id].name})
            .then(response=>{
               ...
               for(let i =0; i<response.data.length; i++){             
                   ...
                   imgg001.src = "http://185.154.53.105:1338/"+response.data[i].main_foto;
